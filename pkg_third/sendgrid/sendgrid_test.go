...
const (
	publicKey = "FkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAE83T4O/n84iotIvIW4mdBgQ/7dAfSmpqIM8kF9mN1flpVKS3GRqe62gw+2fNNRaINXvVpiglSI8eNEc6wEA3F+g=="
	signature = "EUCIGHQVtGj+Y3LkG9fLcxf3qfI10QysgDWmMOVmxG0u6ZUAiEAyBiXDWzM+uOe5W0JuG+luQAbPIqHh89M15TluLtEZtM="
...
func TestEventWebhookNewSettings(t *testing.T) {
	assert.NotNil(t, eventwebhook.NewSettings(), "NewSettings() shouldn't return nil")
}
...
func TestSignedWebhookGetRequestBody(t *testing.T) {
	...
	s.SetEnableSignedWebhook(false)
	actualJSON, err := eventwebhook.GetRequestBody(s)
...
func TestConvertPublicKeyBase64ToECDSA(t *testing.T) {
	ecdsaKey, err := eventwebhook.ConvertPublicKeyBase64ToECDSA(publicKey)
	require.NoError(t, err)
	assert.NotNil(t, ecdsaKey, "publicKey shouldn't be nil")

	ecdsaKey, err = eventwebhook.ConvertPublicKeyBase64ToECDSA(publicKey + "corrupting the public key")
	require.Error(t, err)
...
func TestVerifySignature(t *testing.T) {
	...
	// verifications
	verified, err := eventwebhook.VerifySignature(ecdsaKey, payload, signature, timestamp)
	require.NoError(t, err)
	assert.True(t, verified)

	// not valid payload
	verified, err = eventwebhook.VerifySignature(ecdsaKey, []byte("this is not valid payload for the given signature"), signature, timestamp)
	require.NoError(t, err)
	assert.False(t, verified)

	// not valid signature
	verified, err = eventwebhook.VerifySignature(ecdsaKey, payload, signature+"causing failure", timestamp)
	require.Error(t, err)
	assert.False(t, verified)

	// not valid timestamp
	verified, err = eventwebhook.VerifySignature(ecdsaKey, payload, signature, "invalid timestamp")
	require.NoError(t, err)
	assert.False(t, verified)

	// empty timestamp
	verified, err = eventwebhook.VerifySignature(ecdsaKey, payload, signature, "")
	require.NoError(t, err)
	assert.False(t, verified)
}

func TestVerifySignature01(t *testing.T) {
	ecdsaKey, err := eventwebhook.ConvertPublicKeyBase64ToECDSA(publicKey)
	require.NoError(t, err)
